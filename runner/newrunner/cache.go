...
func NewInputCache(backend ml.Backend, kvCacheType string, kvSize int, numSlots int, multiUserCache bool) (*InputCache, error) {
	...
	cacheType := ml.DTypeF32

	return &InputCache{
		numCtx:         kvSize / numSlots,
		slots:          slots,
		multiUserCache: multiUserCache,
		cache:          cache.NewCausalCache(backend, cacheType, kvSize),
	}, nil
}
...
func (c *InputCache) LoadCacheSlot(prompt []input, cachePrompt bool) (*InputCacheSlot, []input, error) {
	...
	if err != nil {
		...
		err = c.cache.Remove(slot.Id, 0, math.MaxInt)
		if err != nil {
			return nil, nil, err
		}
		numPast = 0
